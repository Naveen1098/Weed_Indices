...
#' @export
#
All_Indices<- function(file_path){
  df <- openxlsx::read.xlsx(file_path,sheet = 1,startRow = 2)
  ...
  if (nrow(WSE_df>0)) {
    WSE1 <-WSE(WSE_df[1,2],WSE_df[2,2])
  }else{warning("There is no data for calculating the Weed smothering efficiency")}

  if (nrow(FDW_df >0)) {
    FDR1 <-FDR(FDW_df[1,2],FDW_df[2,2])
  }else{warning("There is no data for calculating the Weed smothering efficiency")}

  return(list("Weed Indices" = final_indices_df,
              "RWD"=RDW1,
              "WSE"=WSE1,
              "FDR" = FDR1));
}
